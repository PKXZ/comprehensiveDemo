<template>
  <!--互动性地图-->
  <div class="mapfullD">
    <div id="cdCharts" class="mapsGeneral"></div>
    <div id="switchingChart" class="mapsGeneral"></div>
  </div>
</template>

<script>
  import cd from './json/cd.json'
  import cdCity from './json/city/cdCity.json'
  export default {
    name: "switchingMap",
    data(){
      return{
    ...
    methods:{
      drawLine(){
        // 中国地图
        const cdChartContainer = document.getElementById('cdCharts');
        ...
        //点击事件
        cdChart.on("click", function(param) {
          /*
          * @拼接数据
          * @格式：
          *  {
          ...
          * */
          ...
          const cdCityData = cdCity;
          let city = {
            type: "FeatureCollection"
          };
          for(let i = 0; i < cdCityData.features.length; i++){
            if(cdCityData.features[i].properties.name === cityName){
                let data = cdCityData.features[i];
                city['features'] = data;
            }
          }
          self.cityChart(cityName, city);
        })
      ...
      cityChart(name,city){
        ...
        console.log(name);
        console.log(city);
        const switchingChartContainer = document.getElementById('switchingChart');
        const switchingChart = this.$echarts.init(switchingChartContainer);
        this.$echarts.registerMap(name, city);
        // 绘制图表
        const optionMap = {
          backgroundColor: 'rgba(0,51,102, 1)',//地图背景颜色
          //视觉映射组件
        ...
        //switchingChart.showLoading();
        switchingChart.setOption(optionMap,true);//是否不跟之前设置的 option 进行合并，默认为 false，即合并

        window.onresize=function(){
          switchingChart.resize();
          //switchingChart.hideLoading();
        }
      }
    }
  }
